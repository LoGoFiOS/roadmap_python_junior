...
##  Классы. Общее
Класс - механизм создания собственного типа данных. Существуют объекты класса, его экземляров и связанных методов.
...
(3) В namespace `x` нет имени `inc`, но есть в классе. Как же экземпляр `x` получает доступ к нему? Дело в том, что `x.inc()` - связанный метод. Запись `x.inc() == Counter.inc(x)` говорит о том, что `Counter.inc(x)` связалась с `x`.  

Пример с атрибутами общими для всех и индивидуальных для каждого экземляра:
...
## Классы. Наследование
Экземпляр класса может вести себя как экземляр любого класса предка. 
...
Иногда необходимо, чтобы выполнялся не весь код при импортировании. Можно воспользоваться проверкой `if __name__ == "__main__"`. Это условие будет выполняться только тогда, когда модуль будет выполняться в качестве самостоятельного скрипта, а не в качестве импортированного модуля. 

**Важно**. Любой модуль исполняется лишь единожды. Он помещается в словарь sys.modules. При повтороном импортировании возвращается объект из этого словаря. 
...
Итератор - объект перечислитель, возвращающий объект. Ниже показано, как работает итератор: 
...
Можно писать собственные итераторы, определяя "магические" методы `__iter__, __next__`. Можно пользоваться и генераторами.

Генераторы позволяют использовать концепцию "отложенного исполнения" - функция будет продолжена тогда, когда это потребуется. Исполнение тела функции происходит, когда вызывается метод `next`. Происходит от одного `yield` до следующего. Если не найдено, то ошибка `StopIteration`.
...
Модуль `os.path` позволяет выводить папки и файлы, проверять является ли объект папкой или файлом, менять директорию. 
